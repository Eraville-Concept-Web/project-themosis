<?php
	...
	function generate_enqueue_backend_block_editor_assets()
	{
		...
		wp_enqueue_script(
			'generate-block-editor-tinycolor',
			get_template_directory_uri() . '/assets/js/admin/tinycolor.js',
			FALSE,
			GENERATE_VERSION,
			TRUE
		);
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/assets/js/admin/block-editor.js',
			array('jquery', 'generate-block-editor-tinycolor'),
			GENERATE_VERSION,
			TRUE
		);
		...
		if ($show_editor_styles) {
			wp_add_inline_style(
				'generate-block-editor-styles',
				wp_strip_all_tags(generate_do_inline_block_editor_css())
			);
		}
	...
	function generate_do_inline_block_editor_css()
	{
		...
		$css->set_selector(
			'body .wp-block, html body.gutenberg-editor-page .editor-post-title__block, html body.gutenberg-editor-page .editor-default-block-appender, html body.gutenberg-editor-page .editor-block-list__block'
		);
		...
		if (apply_filters('generate_do_group_inner_container_style', TRUE)) {
			...
			$css->add_property(
				'padding',
				generate_padding_css(
					$spacing_settings['content_top'],
					$spacing_settings['content_right'],
					$spacing_settings['content_bottom'],
					$spacing_settings['content_left']
				)
			);
		}
		...
		$css->set_selector(
			'.editor-styles-wrapper a.button:hover, .editor-styles-wrapper a.button:focus, .wp-block-button__link:not(.has-background):active, .wp-block-button__link:not(.has-background):focus, .wp-block-button__link:not(.has-background):hover'
		);
		...
		$buttons_family = generate_get_font_family_css(
			'font_buttons',
			'generate_settings',
			generate_get_default_fonts()
		);
		...
		$css->set_selector(
			'body .editor-styles-wrapper, body .editor-styles-wrapper p, body .editor-styles-wrapper .mce-content-body'
		);
		...
		$css->set_selector(
			'body .editor-styles-wrapper h1, .wp-block-heading h1.editor-rich-text__tinymce, .editor-post-title__block .editor-post-title__input'
		);
		...
		$css->set_selector(
			'.editor-styles-wrapper a.button, .block-editor-block-list__layout .wp-block-button .wp-block-button__link'
		);
		...
		if ($color_settings['content_background_color']) {
			...
			$css->add_property(
				'background',
				'linear-gradient(' . $content_background . ',' . $content_background . '), linear-gradient(' . $body_background . ',' . $body_background . ')'
			);
		}
		...
		$css->set_selector(
			'.block-editor-block-list__block a:hover, .block-editor-block-list__block a:focus, .block-editor-block-list__block a:active'
		);
	...
	/**
	 * Force Richedit to work with stack path
	 */
	function wp_richedit_force()
	{
		add_filter('user_can_richedit', '__return_true');
	}

	Action::add('init', 'wp_richedit_force', 9);
